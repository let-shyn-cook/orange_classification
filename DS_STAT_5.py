...
# Load YOLO model
yolo_model = YOLO('iuo_saved_model/iuo_full_integer_quant.tflite')
...
def detect_and_classify():
    ...
    with open('inference_log.csv', mode='w', newline='') as file:
        writer = csv.writer(file)
        writer.writerow(['Timestamp', 'YOLO Inference Time (s)', 'Keras Inference Time (s)', 'Latency (s)', 'FPS'])
        ...
        try:
            while cap.isOpened():
                ...
                results = yolo_model.predict(source=resized_image, imgsz=224, stream=True, conf=0.7, device='cpu')
                ...
                for track in tracks:
                    if track.is_confirmed():
                        ...
                        keras_latency += time.time() - keras_start_time
                ...
                # Log data to CSV
                writer.writerow([time.strftime("%Y-%m-%d %H:%M:%S"), yolo_latency, keras_latency, frame_time, fps])
